...
void makeListOfAlternativeRecipes(int recipeIndex, Recipe recipes[], Recipe alternativeRecipes[], IngredientData ingredients[], const int ingrNum, Category categories[], const int categoriesLength){
    ...
    for (int t = 0; t < RECIPES_IN_ALTERNATIVES_LIST; t++){
        ...
        scopeIngredientIndexes(ingredientMaxIndexes, ingredientMinIndexes, ingredientStartIndexes, originalRecipe, originalRecipeCoo, ingredientCategories, targets[t], accuracy, &canSatisfyTarget, t);
...
/**
 * @brief Generates a list of well-spaced targets. A target is how many percent co2 (compared to the original recipe) the alternative recipe should emit.
 *
 * @param targets - Array of float targets (length: RECIPES_IN_ALTERNATIVES_LIST)
 */
void generateTargets(float targets[]){
    for(int t = 0; t < RECIPES_IN_ALTERNATIVES_LIST; t++){
